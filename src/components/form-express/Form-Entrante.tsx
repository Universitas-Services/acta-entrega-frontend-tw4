...
export function ActaEntranteForm() {
  ...
  const nextStep = async () => {
    ...
    if (currentStep === 9) {
      ...
      const anexoContent = dynamicStepContentEntrante[selectedAnexoKey];
    ...
    <Card className="w-full bg-white">
      <CardHeader>
        <CardTitle className="text-[24px]">
          {currentStep === 9 && dynamicStepContentEntrante[selectedAnexo]
            ? dynamicStepContentEntrante[selectedAnexo].title
            : steps[currentStep].title}
        </CardTitle>
        <ShadcnCardDescription className="whitespace-pre-line italic font-bold">
          {currentStep === 9 && dynamicStepContentEntrante[selectedAnexo]
            ? dynamicStepContentEntrante[selectedAnexo].subtitle
      ...
      <CardContent>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            {currentStep === 9 && (
              <div>
                {selectedAnexo && dynamicStepContentEntrante[selectedAnexo] ? (
                  (() => {
                    const content = dynamicStepContentEntrante[selectedAnexo];
                    switch (content.type) {
                      case 'questions':
                        return (
                          <div className="space-y-4">
